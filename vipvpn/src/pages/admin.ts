...
export function dashboardPage(): Response {
  ...
  <div class="dash-wrap">
    ...
    <div class="tabs">
      ...
      <button class="tab-btn" data-tab="pool">🖧 پنل‌ها / ورکرهای اضافه</button>
    ...
    <div class="tab-panel" id="tab-pool">
      <div class="glass" style="margin-bottom:1.2rem;">
        <h3 style="margin-bottom:0.6rem;">🖧 پنل‌ها / ورکرهای اضافه</h3>
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          آدرس ورکرهای دیگری که همین پروژه را روی آن‌ها هم دیپلوی کرده‌اید (و به همین دیتابیس D1 وصل کرده‌اید) اینجا اضافه کنید.
          کانفیگ‌های ۵‌گانه کاربران VIP به‌صورت گروهی (batch) بین ورکرهای فعال چرخانده می‌شوند و بقیه در حالت استراحت قرار می‌گیرند —
          مثلاً از ۱۰ ورکر، هر روز فقط ۵ تای آن‌ها فعال‌اند و ۵ تای دیگر استراحت می‌کنند.
        </p>
        <form class="settings-form" id="poolAddForm" style="margin-bottom:1rem;">
          <div class="grid2">
            ...
            <div class="field"><label>برچسب (اختیاری)</label><input type="text" id="poolLabel" placeholder="ورکر ۲" /></div>
          </div>
          <button class="btn" type="submit">➕ افزودن ورکر</button>
        </form>
        ...
        <div class="table-wrap" style="margin-top:1.2rem;">
          <table id="poolTable">
            <thead><tr><th>آدرس</th><th>برچسب</th><th>وضعیت چرخش</th><th>فعال/غیرفعال</th><th>عملیات</th></tr></thead>
            <tbody><tr><td colspan="5">در حال بارگذاری…</td></tr></tbody>
          </table>
        </div>
      </div>
  ...
  (function() {
    ...
    async function loadPool() {
      ...
      if (!data.pool.length) { tbody.innerHTML = '<tr><td colspan="5">هنوز ورکری اضافه نشده — کانفیگ‌های VIP روی همین ورکر باقی می‌مانند</td></tr>'; return; }
      tbody.innerHTML = data.pool.map(w =>
        '<tr>' +
          ...
          '<td class="row-actions"><button class="danger" data-act="delete" data-id="' + w.id + '">حذف</button></td>' +
        '</tr>'
      ).join('');
      tbody.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          try {
            if (act === 'toggle') {
              ...
              if (!confirm('این ورکر از استخر حذف شود؟')) return;
    ...
    loadOverview(); loadUsers(); loadBotUsers(); loadTrialSettings(); loadQuota(); loadPool();
  })();
  </script>`
