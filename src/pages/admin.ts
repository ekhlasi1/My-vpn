...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass" style="margin-bottom:1.2rem;">
        <h3 style="margin-bottom:0.6rem;">🌐 آی‌پی‌های سالم کلادفلر (خودکار، تست از داخل ایران)</h3>
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          این بخش به‌صورت خودکار (هر چند دقیقه) چند آی‌پی از رنج‌های کلادفلر را از طریق سرویس عمومی check-host.net
          و دقیقاً از گره‌های فیزیکی داخل ایران (تهران، اصفهان، شیراز، تبریز، کرج) تست می‌کند، نه از خود ورکر —
          پس نتیجه به واقعیت اتصال کاربران ایرانی نزدیک‌تر است. با این‌حال این گره‌ها روی هاست/دیتاسنتر داخل ایران‌اند،
          نه لزوماً روی آی‌پی خودِ اپراتور موبایل، پس تضمین ۱۰۰٪ برای هر اپراتور/آنتن‌دهی نیست.
        </p>
        <div class="field" style="display:flex; align-items:center; gap:0.5rem; max-width:none;">
          ...
          <span style="font-size:0.8rem; opacity:0.75;">وقتی «روشن» است، کانفیگ‌های تولیدشده فوراً از بهترین آی‌پی سالمِ لیست زیر استفاده می‌کنند؛ وقتی «خاموش» است، کانفیگ‌ها همیشه از دامنه‌ی خود ورکر استفاده می‌کنند و هیچ آی‌پی تست‌شده‌ای در آن‌ها به‌کار نمی‌رود.</span>
        </div>
        <button class="btn" id="cleanIpsRefreshBtn" type="button" style="margin-top:0.6rem;">🔄 تست دسته بعدی آی‌پی‌ها همین الان</button>
  ...
  (function() {
    ...
    async function loadCleanIps() {
      ...
      if (!data.ips.length) { tbody.innerHTML = '<tr><td colspan="4">هنوز هیچ آی‌پی‌ای تست نشده — طی چند دقیقه به‌صورت خودکار شروع می‌شود، یا دکمه بالا را بزنید</td></tr>'; return; }
