...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-trial">
      ...
      <div class="glass">
        <h3>🎯 دامنه تست اسکنر (SNI)</h3>
        <p class="help-text">
          همه‌ی اسکنرهای «آی‌پی تمیز» (اسکن دستی، Radar، اسکنر وحید و دکمه‌ی «تست پینگ» در کانفیگ‌های کاربر) برای
          هر آی‌پی، یک درخواست واقعی با SNI همین دامنه می‌زنند تا ببینند آن آی‌پی اصلاً یک لبه‌ی سالم کلادفلر هست یا
          نه. این دامنه <b>لازم نیست دامنه‌ی خودت یا متصل به این ورکر باشد</b> — کلادفلر بر اساس همین SNI مسیر را
          پیدا می‌کند، نه بر اساس اینکه آن دامنه مال کدام اکانت است؛ کافی‌ست یک دامنه‌ی عمومی و همیشه‌آنلاین باشد که
          پشت کلادفلر است (مثلاً <code>farsroid.com</code>). به همین خاطر این فیلد را می‌توانی خالی بگذاری — به‌طور
          پیش‌فرض از همین دامنه‌ی عمومی استفاده می‌شود و حتی روی یک دیپلوی خام <code>*.workers.dev</code> هم کار
          می‌کند. اگر ترجیح می‌دهی دامنه‌ی دیگری (مال خودت یا هر دامنه‌ی دیگر پشت کلادفلر) استفاده شود، همین‌جا
          واردش کن.
        </p>
        <form class="settings-form" id="scanDomainForm">
          <div class="field" style="max-width:none;">
            <label>دامنه‌ی تست (اختیاری — خالی = farsroid.com)</label>
            <input type="text" id="scanDomain" placeholder="خالی بگذار برای استفاده از farsroid.com" />
          </div>
          <button class="btn" type="submit">💾 ذخیره دامنه اسکنر</button>
        </form>
      </div>
    </div>
  ...
  (function() {
    ...
    async function loadTrialSettings() {
      ...
      document.getElementById('scanDomain').value = data.scan_domain || '';
    }
    ...
    document.getElementById('scanDomainForm').addEventListener('submit', async (e) => {
      ...
      try {
        await api('/api/admin/settings', { method: 'POST', body: JSON.stringify({
          scan_domain: document.getElementById('scanDomain').value.trim(),
        })});
        showToast('دامنه اسکنر ذخیره شد');
      } catch (e) { showToast(e.message, true); }
    });
     ...
     *
     * Keyed by "ip:port" (data-key) rather than array index: the Vahid
     * scanner keeps finding IPs and re-renders this same table every round
     * WHILE it's still running (unlike Radar/manual scan, which render once
     * at the end), and each round's rows are freshly re-sorted by ping — so
     * an index-based checkbox would silently point at a different row after
     * the next round. Previously this also wiped every checkbox on each
     * re-render (innerHTML replace), so ticking an IP mid-scan and then
     * clicking "افزودن" a moment later — the natural way to use a live,
     * still-running scan — landed on an empty selection and errored with
     * "حداقل یک آی‌پی را تیک بزنید" without adding anything. Now checked
     * rows stay checked (matched by ip:port) across re-renders.
     */
    function renderSelectableResults(tableId, rows) {
      ...
      tbody.innerHTML = rows.map((r) => {
        ...
        return (
          '<tr>' +
            '<td><input type="checkbox" class="scanRowCheck" data-key="' + key + '"' + checkedAttr + ' style="width:auto;" /></td>' +
            '<td>' + countryCellHtml(r.country) + '</td>' +
            '<td style="font-family:monospace;">' + r.ip + '</td>' +
            '<td style="font-family:monospace;">' + r.port + '</td>' +
    ...
    /** Wires up an "add selected" button for a (tableId, rows-getter) pair, shared by Radar + manual scan + Vahid scan. */
    function setupBulkAddButton(btnId, tableId, getRows) {
      document.getElementById(btnId).addEventListener('click', async () => {
        const rows = getRows();
        const rowsByKey = new Map(rows.map((r) => [r.ip + ':' + r.port, r]));
        const checked = Array.from(document.querySelectorAll('#' + tableId + ' .scanRowCheck:checked'))
          .map((el) => rowsByKey.get(el.dataset.key))
          .filter(Boolean);
