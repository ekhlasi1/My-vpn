...
export function dashboardPage(): Response {
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass" style="margin-bottom:1.2rem;">
        ...
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          ...
          از خود ورکر تست می‌کند. بهترین آی‌پی سالم به‌عنوان آدرس اتصال (add) در همه‌ی کانفیگ‌های تحویلی به کاربران
          جایگزین می‌شود — بدون تغییر در sni/host، پس مسیریابی هرگز خراب نمی‌شود.
        </p>
        <button class="btn" id="cleanIpsRefreshBtn" type="button">🔄 تست دسته بعدی آی‌پی‌ها همین الان</button>
  ...
  (function() {
    ...
    loadOverview(); loadUsers(); loadBotUsers(); loadTrialSettings(); loadQuota(); loadPool(); loadSyncSecret(); loadCleanIps(); loadDnsRules();
  })();
  </script>`
