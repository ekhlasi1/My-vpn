...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-trial">
      ...
      <div class="glass">
        <h3>🧱 Proxy IP — باز شدن سایت‌های پشت کلادفلر</h3>
        <p class="help-text">
          کلادفلر ورکرز اجازه نمی‌دهد مستقیماً به رنج آی‌پی خودِ کلادفلر (یعنی اکثر سایت‌های بزرگ که پشت CDN/WAF
          کلادفلر هستند) وصل شوی؛ برای همین این ورکر برای این‌جور مقصدها از یک «Proxy IP» به‌عنوان واسط استفاده
          می‌کند. این فیلد <b>دیگر نیازی به پر کردن دستی ندارد</b>: هر بار که از هر کدام از اسکنرهای بالا (Radar /
          اسکن دستی / اسکنر وحید) نتیجه‌ای را تیک می‌زنی و به «لیست سرورهای کاربران» اضافه می‌کنی، همان آی‌پی‌ها
          خودکار اینجا هم اضافه می‌شوند (جدیدترین‌ها اول، حداکثر ۵ مورد، بدون نیاز به هیچ کار دستی یا ری‌دیپلوی) —
          پس همین که اسکن می‌زنی و اضافه می‌کنی، همان لحظه سایت‌های پشت کلادفلر هم باید بهتر باز شوند. این فیلد
          فقط برای دیدن مقدار فعلی یا override دستی (مثلاً یک Proxy IP اختصاصی که خودت جدا از اسکنرها داری) نگه
          داشته شده؛ اگر خالی بماند از همان آی‌پی‌های خودکارِ بالا (یا در نبود آن‌ها، از <code>PROXY_IP</code> در
          تنظیمات ورکر) استفاده می‌شود.
        </p>
        <form class="settings-form" id="proxyIpForm">
          <div class="field" style="max-width:none;">
            <label>Proxy IP دستی/override (یک یا چند مورد، جدا با ویرگول — مثلاً <code>1.2.3.4:443,5.6.7.8:2053</code>)</label>
            <input type="text" id="proxyIpPool" placeholder="خالی = استفاده خودکار از نتایج اسکنرها" />
          </div>
          <button class="btn" type="submit">💾 ذخیره Proxy IP</button>
        </form>
      </div>
    </div>
  ...
  (function() {
    ...
    async function api(path, opts) {
      ...
      let res;
      try {
        const requestOpts = Object.assign({ headers: { 'Content-Type': 'application/json' }, signal: controller.signal }, opts || {});
        if (opts?.signal) {
          ...
          try {
            res = await fetch(path, requestOpts);
          } finally {
      ...
      if (!res.ok) throw new Error(data.error || ('خطا (کد ' + res.status + ')'));
      return data;
    }
    ...
    async function loadTrialSettings() {
      ...
      document.getElementById('proxyIpPool').value = data.proxy_ip_pool || '';
    }
    ...
    document.getElementById('proxyIpForm').addEventListener('submit', async (e) => {
      e.preventDefault();
      try {
        await api('/api/admin/settings', { method: 'POST', body: JSON.stringify({
          proxy_ip_pool: document.getElementById('proxyIpPool').value,
        })});
    ...
    /**
     * Used where a results table can re-render WHILE more results are still
     * coming in (currently just the Vahid scanner — Radar/manual scan only
     * render once, after their scan is done, so this never applies there).
     * A plain top-N-by-ping slice would silently evict an already-ticked
     * row the moment enough faster hits came in to push it past position N
     * — the row (and its checkbox) just disappears on the next render, so a
     * selection made earlier in a still-running scan could vanish before
     * the admin got to click "افزودن". This keeps any row the admin has
     * currently checked in the rendered list even if a plain ping-sort
     * would have dropped it, so ticking something never loses it out from
     * under you while the scan keeps running.
     */
    function withPinnedSelections(tableId, topRows, allRows) {
      const checkedKeys = new Set(
        Array.from(document.querySelectorAll('#' + tableId + ' .scanRowCheck:checked')).map((el) => el.dataset.key),
      );
      if (!checkedKeys.size) return topRows;
      const present = new Set(topRows.map((r) => r.ip + ':' + r.port));
      const pinned = allRows.filter((r) => checkedKeys.has(r.ip + ':' + r.port) && !present.has(r.ip + ':' + r.port));
      if (!pinned.length) return topRows;
      return [...topRows, ...pinned].sort((a, b) => (a.ping ?? 999999) - (b.ping ?? 999999));
    }

    /**
     * Renders "rows" (already top-15-filtered) into "tableId" with one checkbox per row.
     *
     * Each checkbox carries its row's full data as data-* attributes
     * (ip/port/ping/country) instead of an array index or a lookup key that
     * has to be resolved back against some external array later. The Vahid
     * scanner re-renders this same table every round WHILE it's still
     * running (unlike Radar/manual scan, which render once at the end), so
     * anything that required looking a checked box back up in "the current
     * rows array" at click time was fragile in two ways: (1) index-based
     * lookups broke because each round re-sorts by ping, and (2) even a
     * key-based lookup broke if a round found enough better (lower-ping)
     * hits to push an already-ticked IP out of the top-15 window — the row
     * (and its checkbox) would vanish from the next render entirely, so by
     * the time "افزودن" was clicked the tick was just gone and it errored
     * with "حداقل یک آی‌پی را تیک بزنید" without adding anything, even
     * though the admin had genuinely ticked a healthy result. Embedding the
     * row's data directly on the checkbox removes the lookup entirely; see
     * the Vahid round loop below for how already-checked rows are also now
     * kept in the rendered list instead of being silently evicted.
     */
    function renderSelectableResults(tableId, rows) {
      const table = document.getElementById(tableId);
      const tbody = table.querySelector('tbody');
      const previouslyChecked = new Set(
        Array.from(tbody.querySelectorAll('.scanRowCheck:checked')).map((el) => el.dataset.key),
      );
      table.style.display = rows.length ? 'table' : 'none';
      tbody.innerHTML = rows.map((r) => {
        const key = r.ip + ':' + r.port;
        const checkedAttr = previouslyChecked.has(key) ? ' checked' : '';
        return (
          '<tr>' +
            '<td><input type="checkbox" class="scanRowCheck"' +
              ' data-key="' + key + '" data-ip="' + r.ip + '" data-port="' + r.port + '"' +
              ' data-ping="' + r.ping + '" data-country="' + (r.country || '') + '"' +
              checkedAttr + ' style="width:auto;" /></td>' +
            '<td>' + countryCellHtml(r.country) + '</td>' +
            '<td style="font-family:monospace;">' + r.ip + '</td>' +
            '<td style="font-family:monospace;">' + r.port + '</td>' +
    ...
    /**
     * Wires up an "add selected" button, shared by Radar + manual scan + Vahid scan.
     * Reads selection straight off the checked checkboxes' own data-*
     * attributes — no dependency on whatever array happens to be "current"
     * at click time (see renderSelectableResults' comment for why that used
     * to silently drop selections during a still-running Vahid scan).
     */
    function setupBulkAddButton(btnId, tableId) {
      document.getElementById(btnId).addEventListener('click', async () => {
        const checked = Array.from(document.querySelectorAll('#' + tableId + ' .scanRowCheck:checked')).map((el) => ({
          ip: el.dataset.ip, port: Number(el.dataset.port), ping: Number(el.dataset.ping), country: el.dataset.country || '',
        }));
    ...
    setupBulkAddButton('candidateAddSelectedBtn', 'candidateResultsTable');

    let lastRadarTop15 = [];
    setupBulkAddButton('radarAddSelectedBtn', 'radarResultsTable');
    ...
    //
    // IMPORTANT: with mode:'no-cors', a genuinely open port resolves the
    // fetch (opaque response) — it does NOT throw. So landing in catch
    // below always means a real failure: DNS error, TCP refused/reset, a
    // TLS failure, or our own timeout firing. Every one of those means the
    // port is NOT usably open. This used to return
    // open: !controller.signal.aborted — i.e. "open" for every failure
    // EXCEPT our own timeout, which is backwards: a port that fails fast
    // (refused/reset) got counted as open, and only a port that hung for
    // the full timeout was counted closed. Since a real, working port needs
    // an actual TLS round trip while a closed one typically errors out
    // almost instantly, that inversion systematically favored fast-failing
    // (closed) ports as "fastest ping" over genuinely open ones — which is
    // a very plausible reason the scanner kept surfacing the same one port
    // (whichever tends to answer/refuse fastest on a given network) no
    // matter which ports were ticked. Fixed: any failure means closed.
    ...
    // Latency of ONE specific port (was hard-wired to 443, which is why every
    // result showed port 443 no matter which ports were ticked).
    async function vahidGetLatencyMs(ip, port, signal) {
      const { elapsed } = await vahidProbe(ip, port, VAHID_PING_TIMEOUT_MS, signal);
      return elapsed >= VAHID_PING_TIMEOUT_MS ? null : Math.round(elapsed);
    }

    // Times every port that answered and returns the fastest one, so the
    // reported port/ping always belong together and are always one of the
    // ports the admin actually ticked.
    async function vahidBestOpenPort(ip, openPorts, signal) {
      const timed = await Promise.all(openPorts.map(async (port) => ({
        port, ping: await vahidGetLatencyMs(ip, port, signal),
      })));
      const ok = timed.filter((t) => t.ping != null).sort((a, b) => a.ping - b.ping);
      return ok[0] || null;
    }
      ...
      for (let i = 0; i < VAHID_SCAN_ATTEMPTS_PER_FINDER; i++) {
        ...
        if (ip) {
          ...
          if (openPorts.length > 0) {
            const best = await vahidBestOpenPort(ip, openPorts, signal);
            if (best) return { success: true, ip, port: best.port, ping: best.ping, country: null };
          }
        }
    ...
    async function runVahidScanRoundClient(ports, signal) {
      ...
      const data = await api('/api/admin/vahid-scan/round', {
        method: 'POST',
    ...
    setupBulkAddButton('vahidScanAddSelectedBtn', 'vahidScanResultsTable');
    ...
    document.getElementById('vahidScanBtn').addEventListener('click', async () => {
      ...
      try {
        for (let round = 0; round < MAX_ROUNDS && collected.length < TARGET; round++) {
          ...
          lastVahidScanTop15 = withPinnedSelections('vahidScanResultsTable', topHealthyByPing(collected), collected);
