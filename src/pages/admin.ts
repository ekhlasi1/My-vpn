...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass" style="margin-bottom:1.2rem;">
        <h3 style="margin-bottom:0.6rem;">🌐 آی‌پی‌های سالم کلادفلر (خودکار)</h3>
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          این بخش به‌صورت خودکار (هر ساعت، همراه با نگهداری زمان‌بندی‌شده‌ی پروژه) چند آی‌پی از رنج‌های کلادفلر را مستقیماً
          از خود ورکر تست می‌کند. این تست فقط اتصال «کلادفلر به کلادفلر» را می‌سنجد — یعنی نمی‌تواند تشخیص بدهد که آیا
          آن آی‌پی از داخل ایران هم باز است یا نه، پس ممکن است یک آی‌پی را «سالم» نشان بدهد در حالی که برای کاربران ایرانی
          فیلتر است. به همین دلیل جایگزینی خودکار آدرس اتصال (add) پیش‌فرض <b>غیرفعال</b> است و کانفیگ‌ها به‌صورت عادی
          از دامنه‌ی خود ورکر استفاده می‌کنند. فقط در صورتی فعالش کنید که خودتان از سالم‌بودن واقعی آی‌پی‌های این لیست
          برای کاربران ایران مطمئن شده‌اید.
        </p>
        <div class="field" style="display:flex; align-items:center; gap:0.5rem; max-width:none;">
          ...
          <label for="autoCleanIp" style="margin:0;">جایگزینی خودکار آی‌پی تست‌شده به‌جای دامنه در کانفیگ‌ها (پیشنهاد نمی‌شود)</label>
        </div>
        <button class="btn" id="autoCleanIpSaveBtn" type="button" style="margin-top:0.6rem;">💾 ذخیره</button>
        <button class="btn" id="cleanIpsRefreshBtn" type="button" style="margin-inline-start:0.5rem; margin-top:0.6rem;">🔄 تست دسته بعدی آی‌پی‌ها همین الان</button>
        <div class="table-wrap" style="margin-top:1rem;">
          <table id="cleanIpsTable">
            <thead><tr><th>آی‌پی</th><th>وضعیت</th><th>تأخیر (ms)</th><th>آخرین بررسی</th></tr></thead>
            <tbody><tr><td colspan="4">در حال بارگذاری…</td></tr></tbody>
          </table>
        </div>
      </div>
    </div>
  ...
  (function() {
    ...
    async function loadAutoCleanIpToggle() {
      ...
      document.getElementById('autoCleanIp').checked = settings.auto_clean_ip_enabled === '1';
    }
    document.getElementById('autoCleanIpSaveBtn').addEventListener('click', async () => {
      try {
        await api('/api/admin/settings', { method: 'POST', body: JSON.stringify({
          auto_clean_ip_enabled: document.getElementById('autoCleanIp').checked ? '1' : '0',
        })});
        showToast('ذخیره شد');
      } catch (e) { showToast(e.message, true); }
    });
    async function loadCleanIps() {
      ...
      if (!data.ips.length) { tbody.innerHTML = '<tr><td colspan="4">هنوز هیچ آی‌پی‌ای تست نشده — طی یک ساعت به‌صورت خودکار شروع می‌شود، یا دکمه بالا را بزنید</td></tr>'; return; }
    ...
    document.getElementById('cleanIpsRefreshBtn').addEventListener('click', async (e) => {
      const btn = e.target;
      btn.disabled = true; const orig = btn.textContent; btn.textContent = 'در حال تست…';
      try {
        await api('/api/admin/clean-ips/refresh', { method: 'POST' });
        showToast('دسته بعدی تست شد');
        loadCleanIps();
      } catch (e) { showToast(e.message, true); }
      finally { btn.disabled = false; btn.textContent = orig; }
    });
