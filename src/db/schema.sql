...
CREATE TABLE IF NOT EXISTS backend_pool (
  ...
  country          TEXT                    -- optional, manual, cosmetic-only label (v11) — see src/utils/countries.ts
);

-- Manually-entered "healthy IP:port" list, per requirement #5. No scanning
-- or automatic testing of any kind — the admin pastes in IP:port pairs
-- they've personally confirmed work for their users from the dashboard's
-- "پنل‌ها" tab. The best row (picked at random among all entries) is used as
-- the VLESS `add=` (connect) address for every generated config, while
-- `host`/`sni` stay on the real worker domain — see db/queries.ts and
-- requirement #4 (minimizing IP-level filtering without breaking
-- Cloudflare's SNI/Host based routing).
...
-- Placeholders: {brand}, {admin}, {n} (pro only), {flag}/{country} (pro only,
-- from that config's backend_pool.country tag — see migration-v11.sql).
INSERT OR IGNORE INTO settings (key, value) VALUES ('pro_config_name', '{flag} {brand} VIP |{country}{n} | @{admin}');
