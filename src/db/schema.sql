...
CREATE TABLE IF NOT EXISTS backend_pool (
  ...
  cf_account_id    TEXT,                   -- Cloudflare Account ID (optional)
  cf_api_token     TEXT,                   -- Cloudflare API Token scoped to that account (optional)
  cf_database_id   TEXT,                   -- that account's own D1 database id, for user-sync (optional)
  cf_script_name   TEXT,                   -- the deployed Worker script name on that account (optional)
