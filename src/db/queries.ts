...
/** Persists the results of one discoverCleanIpsBatch() run. */
async function saveCleanIpResults(env: Env, results: CleanIpResult[]): Promise<void> {
  for (const r of results) {
    await env.DB.prepare(
      `INSERT INTO clean_ips (ip, latency_ms, healthy, last_checked, last_error)
       VALUES (?, ?, ?, ?, ?)
      ...
      .bind(r.ip, r.latencyMs, r.healthy ? 1 : 0, Date.now(), r.error)
      .run()
  }
}
 ...
 */
export async function runCleanIpDiscovery(env: Env, domain: string): Promise<void> {
  const offsetStr = await getSetting(env, 'clean_ip_scan_offset')
  const offset = parseInt(offsetStr, 10) || 0
  const results = await discoverCleanIpsBatch(domain, offset)
 ...
 * options. Returns null if none has been discovered yet (fresh deploy) or
 * none currently qualifies — callers must treat null as "no override", i.e.
 * configs fall back to the worker's own domain as the connect address
 * exactly like before this feature existed.
 */
export async function getBestCleanIp(env: Env): Promise<string | null> {
  const { results } = await env.DB.prepare(
    `SELECT ip FROM clean_ips WHERE healthy = 1 AND last_checked > ? ORDER BY latency_ms ASC LIMIT 5`,
  )
