...
export async function addPoolWorker(env: Env, input: PoolWorkerInput): Promise<void> {
  await env.DB.prepare(
    `INSERT INTO backend_pool
      (url, label, enabled, added_at, cf_account_id, cf_api_token, cf_database_id, cf_script_name, health_status)
     VALUES (?, ?, 1, ?, ?, ?, ?, ?, 'unknown')`,
  )
    .bind(
      input.url,
...
/**
 * Mirrors one VIP/trial user into every enabled account that has full D1
 * credentials configured, so the subscription UUID this project just issued
 * is also valid on those other accounts' workers. Called right after a user
 * is created/renewed/edited (see telegram/bot.ts and api/admin.ts). Never
 * throws — a sync failure on one account is logged and skipped, it must
 * never break issuing the subscription on the primary account.
 */
export async function syncUserToAllAccounts(env: Env, user: SyncableUser): Promise<void> {
  ...
  const targets = accounts.filter((a) => a.enabled && a.cf_api_token && a.cf_account_id && a.cf_database_id)
  await Promise.all(
...
/** Runs (and stores) a live health check for one account — see checkAccountHealth() in cf-accounts.ts. */
export async function checkAndStorePoolWorkerHealth(env: Env, id: number): Promise<PoolWorkerRow | null> {
  ...
  if (!row) return null
  const result = await checkAccountHealth(toCfTarget(row))
