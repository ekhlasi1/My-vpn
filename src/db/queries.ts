...
import { RENTED_CLEAN_IPS, type RentedCleanIp } from '../data/rented-clean-ips'
...
const SETTINGS_DEFAULTS: Record<string, string> = {
  ...
  // {flag}/{country} come from the position's backend_pool.country tag
  // (v11) — both render empty when that pool account has no country set,
  // which quietly collapses back to the old "| N |" look.
  pro_config_name: '{flag} {brand} VIP |{country}{n} | @{admin}',
  ...
  pool_batch_size: '5',
  pool_rest_days: '1',
  // ---- Requirement #5: manually-entered "healthy IP" list ----
  // This used to be an automatic scanner that tested Cloudflare candidate
  // IPs from inside the Worker itself — which could only ever measure
  // Cloudflare-to-Cloudflare reachability, not whether a given ISP could
  // actually reach that IP, producing "shows healthy but doesn't work for
  // users" results. It's been replaced entirely: the admin now enters
  // IP:port pairs they've personally confirmed work (see the `clean_ips`
  // table / listCleanIps / addCleanIp below). Off by default so configs
  // keep using the worker's own domain as the connect address until the
  // admin has entered at least one confirmed-working entry and turned this
  // on.
  clean_ip_override_enabled: '0',
  ...
  wiki_gift_link: '',
  // ---- Weekly "servers updated / hosting paid" broadcast to all bot users ----
  weekly_update_broadcast_enabled: '1',
  last_weekly_update_broadcast_at: '0',
}
...
/**
 * Builds a random path segment for the owner's personal subscription link:
 * the brand name (lowercased, alnum-only, falls back to "bndmax" if the
 * admin cleared it) followed by 4 random digits, e.g. "bndmax7421". Not
 * meant to be cryptographically unguessable on its own (4 digits is only
 * 10,000 combinations) — it's meant to replace a permanently-fixed,
 * publicly-documented path ("/sub") with something that isn't the same on
 * every install and can be rotated instantly if it ever leaks, the same
 * "don't publish a fixed admin path" idea as /admin being renamed on other
 * panels. Combined with the fact that this path resolves to a config that's
 * outside the tracked-user system entirely (unlimited volume/time), it
 * should still be treated as a secret the admin doesn't share.
 */
function generateOwnerSubPathValue(brand: string): string {
  const slug = (brand || '').toLowerCase().replace(/[^a-z0-9]/g, '') || 'bndmax'
...
/**
 * The owner's random subscription path, generated lazily on first read
 * (using the current brand_name setting) so upgraders don't need a manual
 * migration step, then persisted so it stays stable across requests/deploys
 * until the admin explicitly regenerates it.
 */
export async function getOrCreateOwnerSubPath(env: Env): Promise<string> {
  const existing = await getSetting(env, 'owner_sub_path')
  if (existing) return existing
  const brand = await getSetting(env, 'brand_name')
  const fresh = generateOwnerSubPathValue(brand)
  await setSetting(env, 'owner_sub_path', fresh)
  return fresh
}

/** Issues a brand-new random path, invalidating the old one immediately. */
export async function regenerateOwnerSubPath(env: Env): Promise<string> {
  const brand = await getSetting(env, 'brand_name')
  const fresh = generateOwnerSubPathValue(brand)
  await setSetting(env, 'owner_sub_path', fresh)
  return fresh
...
/** Fast aggregate stats for the dashboard. Avoids loading thousands of user rows just to render six counters. */
export async function getUserStats(env: Env): Promise<UserStats> {
  const row = await env.DB.prepare(
    `SELECT
       COUNT(*) AS total,
       COALESCE(SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END), 0) AS active,
       COALESCE(SUM(CASE WHEN type = 'pro' THEN 1 ELSE 0 END), 0) AS pro,
       COALESCE(SUM(CASE WHEN type = 'trial' THEN 1 ELSE 0 END), 0) AS trial,
       COALESCE(SUM(CASE WHEN status != 'active' THEN 1 ELSE 0 END), 0) AS inactive,
       COALESCE(SUM(volume_used_mb), 0) AS totalUsageMb
     FROM users`,
  ).first<{
    total: number
    active: number
  ...
  return {
    ...
    totalUsageMb: Number(row?.totalUsageMb) || 0,
  }
}
 ...
 * Returns true if the connection should be allowed to proceed.
 *
 * This used to read the counter (getAllSettings), add 1 in JS, then write it
 * back (setSettings) as two separate round trips. Under real concurrent
 * traffic (many simultaneous proxy connections, which is the normal case for
 * this worker) that read-then-write was a classic lost-update race: two
 * requests could both read the same count N, both compute N+1, and both
 * write N+1 — so two increments landed as one and the daily counter shown
 * in the dashboard drifted further below the real request count the more
 * concurrent traffic there was. The counter update below is now a single
 * atomic UPDATE (one D1 round trip, so nothing can interleave inside it)
 * that does the day-rollover check and the +1 in the same statement and
 * returns the authoritative post-increment value via RETURNING.
 */
export async function checkAndIncrementQuota(env: Env): Promise<{ allowed: boolean; paused: boolean }> {
  const paused = await getSetting(env, 'service_paused')
  if (paused === '1') return { allowed: false, paused: true }

  const today = todayUtc()
  const limitStr = await getSetting(env, 'daily_request_limit')
  const autoPauseStr = await getSetting(env, 'auto_pause_at_limit')
  const limit = parseFloat(limitStr) || 90000
  const autoPause = autoPauseStr !== '0'

  const row = await env.DB.prepare(
    `INSERT INTO settings (key, value) VALUES ('usage_quota_count', '1')
     ON CONFLICT(key) DO UPDATE SET value = CASE
       WHEN (SELECT value FROM settings WHERE key = 'usage_quota_date') = ?1
         THEN CAST(CAST(settings.value AS INTEGER) + 1 AS TEXT)
       ELSE '1'
     END
     RETURNING value`,
  )
    .bind(today)
    .first<{ value: string }>()
  const nextCount = parseFloat(row?.value ?? '1') || 1

  // Not part of the count's own atomicity — this just records "today" as the
  // last-seen day, and re-writing the same string on a race is harmless.
  await setSetting(env, 'usage_quota_date', today)

  const overLimit = limit > 0 && nextCount >= limit
  if (overLimit && autoPause) await setSetting(env, 'service_paused', '1')
...
export interface PoolWorkerRow {
  ...
  // ---- Optional country tag (v11), purely cosmetic — see src/utils/countries.ts.
  // Lets pro/VIP config names show a flag + country name for whichever
  // account each config's connection is pointed at, instead of the generic
  // "سرور N". Never used for routing/selection logic.
  country: string | null
}
...
export async function addPoolWorker(env: Env, input: PoolWorkerInput): Promise<void> {
  const inserted = await env.DB.prepare(
    `INSERT INTO backend_pool
      (url, label, enabled, added_at, cf_account_id, cf_api_token, cf_database_id, cf_script_name, sync_secret, health_status, country)
     VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?, 'unknown', ?)
     RETURNING *`,
  )
    .bind(
 ...
 */
export interface ActivePoolHost {
  hostname: string
  /** Cosmetic country tag from that account's backend_pool.country (v11), or null if untagged. */
  country: string | null
}

export async function getPoolRotation(
  env: Env,
): Promise<{ activeHosts: ActivePoolHost[]; pool: (PoolWorkerRow & { active: boolean; hostname: string })[] }> {
  ...
  const activeHosts: ActivePoolHost[] = pool
    .filter((w) => w.enabled && w.active && w.health_status !== 'unhealthy')
    .map((w) => ({ hostname: w.hostname, country: w.country ?? null }))

  return { activeHosts, pool }
}

// ==================== Manually-entered "healthy IP" list (requirement #5) ====================
//
// There is no scanning/testing of any kind here anymore. The admin pastes in
// IP:port pairs they've personally confirmed work for their users, from the
// dashboard's "پنل‌ها" tab; that's the entire feature.
//
// v10 adds an optional `country` label (cosmetic only — still no scanning),
// plus RENTED_CLEAN_IPS (src/data/rented-clean-ips.ts): a small code-level
// list for servers the admin personally rents/owns and rotates often, so
// updating them is a one-line file edit + redeploy instead of a dashboard
// round-trip. Both sources are merged for display and for getBestCleanIp().
...
export interface CleanIpRow {
  ...
  country: string | null
  created_at: number
  /** true when this row comes from the code-level rented-clean-ips.ts file (not deletable via the API). */
  fromFile?: boolean
}

function rentedToRow(entry: RentedCleanIp, index: number): CleanIpRow {
  return {
    id: `file:${index}`,
    ip: entry.ip,
    port: entry.port,
...
/** All admin-entered healthy IPs (DB + rented-clean-ips.ts file), most recently added first. */
export async function listCleanIps(env: Env, limit = 200): Promise<CleanIpRow[]> {
  const { results } = await env.DB.prepare(
    `SELECT * FROM clean_ips ORDER BY created_at DESC LIMIT ?`,
  )
    .bind(limit)
    .all<CleanIpRow>()
  const fileRows = RENTED_CLEAN_IPS.map(rentedToRow)
  return [...fileRows, ...(results ?? [])]
}

/**
 * Max number of admin-entered rows (D1 only — RENTED_CLEAN_IPS from the
 * file is a separate, code-level list and isn't subject to this) kept in
 * the "healthy IP" list at once. This is also the max number of extra,
 * per-IP configs (see subscription.ts's buildUserSubscription) shown to
 * each managed user on top of the PRO_CONFIG_COUNT automatic ones.
 * Enforced on every insert: once the list is full, the oldest row is
 * evicted first, so a newly-picked IP from the Radar/manual scanner
 * naturally rotates out the stalest entry instead of growing forever.
 */
export const MAX_CLEAN_IPS = 15

async function enforceCleanIpCap(env: Env): Promise<void> {
  const { results } = await env.DB.prepare(`SELECT id FROM clean_ips ORDER BY created_at ASC`).all<{ id: number }>()
  const rows = results ?? []
  const excess = rows.length - MAX_CLEAN_IPS
  if (excess <= 0) return
  for (const row of rows.slice(0, excess)) {
    await env.DB.prepare(`DELETE FROM clean_ips WHERE id = ?`).bind(row.id).run()
  }
}

/** Adds one admin-entered IP:port entry to the healthy-IP list, evicting the oldest row if this pushes the list past MAX_CLEAN_IPS. */
export async function addCleanIp(env: Env, ip: string, port: number, note?: string | null, country?: string | null): Promise<void> {
  await env.DB.prepare(
    `INSERT INTO clean_ips (ip, port, note, country, created_at)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(ip, port) DO UPDATE SET note = excluded.note, country = excluded.country`,
  )
    .bind(ip, port, note || null, country || null, Date.now())
    .run()
  await enforceCleanIpCap(env)
}

/** Adds several IP:port entries at once (e.g. from the scanner's "add selected" button). Returns how many were accepted. */
export async function addCleanIps(
  env: Env,
  entries: { ip: string; port: number; note?: string | null; country?: string | null }[],
): Promise<number> {
  let added = 0
  for (const entry of entries) {
    const ip = String(entry.ip || '').trim()
    if (!ip) continue
    const port = Number(entry.port) || 443
    if (port < 1 || port > 65535) continue
    await addCleanIp(env, ip, port, entry.note ?? null, entry.country ?? null)
    added++
  }
  return added
}
...
/**
 * Automation for the "کاشکی این مراحل خودکار انجام می‌شد" request: every time
 * the admin adds scanner results (Radar / manual scan / Vahid scan) to the
 * user-facing clean-IP list, the same IPs are now also folded into
 * `proxy_ip_pool` (see resolveProxyIpPool in services/access.ts) — the
 * server-side relay used to reach sites behind Cloudflare. Before this, that
 * setting could only be updated by hand on the dashboard. Newest scan results
 * go first (retry() in tcp.ts tries the pool in order), duplicates are
 * dropped, and the pool is capped at PROXY_POOL_AUTO_MAX so it stays made of
 * genuinely fresh, recently-verified addresses rather than growing forever.
 */
const PROXY_POOL_AUTO_MAX = 5

export async function addToProxyIpPoolFromScan(
  env: Env,
  entries: { ip: string; port: number }[],
): Promise<void> {
  if (!entries.length) return
  const incoming = entries.map((e) => `${String(e.ip).trim()}:${Number(e.port) || 443}`)
  const current = splitAndFilter(await getSetting(env, 'proxy_ip_pool'), ',')
  const merged: string[] = []
  for (const item of [...incoming, ...current]) {
    if (!merged.includes(item)) merged.push(item)
  }
  await setSetting(env, 'proxy_ip_pool', merged.slice(0, PROXY_POOL_AUTO_MAX).join(','))
}

/** Removes one entry from the healthy-IP list. Rows from rented-clean-ips.ts (id like "file:0") aren't in the DB — edit that file + redeploy to change/remove them instead. */
export async function deleteCleanIp(env: Env, id: number): Promise<void> {
  await env.DB.prepare(`DELETE FROM clean_ips WHERE id = ?`).bind(id).run()
}

/**
 * Picks one admin-entered healthy IP:port at random (so all users' traffic
 * doesn't concentrate on a single address), gated behind
 * clean_ip_override_enabled (default '0'/off — see the setting's comment
 * above). Returns null when the toggle is off or the list is empty, meaning
 * "no override": callers fall back to the worker's own domain (and port 443)
 * as the connect address, exactly like before this feature existed.
 */
export async function getBestCleanIp(env: Env): Promise<{ ip: string; port: number } | null> {
  const enabled = await getSetting(env, 'clean_ip_override_enabled')
  if (enabled !== '1') return null

  const { results } = await env.DB.prepare(`SELECT ip, port FROM clean_ips`).all<{ ip: string; port: number }>()
  const pool = [...RENTED_CLEAN_IPS.map((r) => ({ ip: r.ip, port: r.port })), ...(results ?? [])]
