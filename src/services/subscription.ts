 ...
 *  - trial users: a single config, named after them.
...
export function buildUserSubscription(
  ...
  const buyer = user.telegram_name || user.telegram_id || user.uuid.slice(0, 8)
  ...
  if (user.type === 'pro') {
    ...
    for (let i = 0; i < PRO_CONFIG_COUNT; i++) {
      const name = `👑 VIP ${brand} | ${buyer} | سرور ${i + 1}`
    ...
    const baseName = `${brand} | ${buyer} | خرید: @${adminUsername}`
    entries.push({ name: baseName, link: generateSubscription(user.uuid, url, baseName) })
  }
