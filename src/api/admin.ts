...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // Requirement #2: quick, reliable "extract configs from another account's
  // worker" — given just the pool row already saved (its URL + sync secret)
  // and a target uuid, re-syncs that one user there right now and returns
  // that worker's own real, live subscription lines for it. See
  // extractConfigFromPoolWorker in db/queries.ts.
  const extractMatch = path.match(/^\/pool\/(\d+)\/extract-config$/)
  if (extractMatch && method === 'POST') {
    const id = Number(extractMatch[1])
    const { uuid } = await readJson(request)
    if (!uuid || typeof uuid !== 'string') return json({ error: 'یک کاربر را انتخاب کنید' }, 400)
    try {
      const { links, hostname } = await extractConfigFromPoolWorker(env, id, uuid)
      return json({ ok: true, links, hostname })
    } catch (err) {
      return json({ error: err instanceof Error ? err.message : 'استخراج کانفیگ ناموفق بود' }, 502)
    }
  }
  ...
  if (path === '/clean-ips/refresh' && method === 'POST') {
    // Admin-controlled test count ("تعداد آی‌پی که تست می‌شود") — clamped to
    // a sane range so a typo can't queue up hundreds of sequential
    // check-host.net calls in one click. Testing itself runs one IP at a
    // time (see discoverCleanIpsBatch) to stay reliable against check-host's
    // shared rate limit and to give real "one after another" progress.
    const body = await readJson(request)
    const requestedCount = Number(body?.count)
    const count = Number.isFinite(requestedCount) ? Math.max(1, Math.min(30, Math.round(requestedCount))) : undefined
    await runCleanIpDiscovery(env, url.hostname, count)
    const ips = await listCleanIps(env)
    return json({ ok: true, ips })
  }

  // Manual, final-confirmation step: admin selects rows from the tested
  // list and approves (or unapproves) them; only approved+healthy IPs are
  // ever eligible in getBestCleanIp (see its comment in queries.ts).
  if (path === '/clean-ips/approve' && method === 'POST') {
    const { ips, approved } = await readJson(request)
    if (!Array.isArray(ips) || ips.length === 0) return json({ error: 'حداقل یک آی‌پی را انتخاب کنید' }, 400)
    await setCleanIpsApproved(env, ips.map(String), approved !== false)
    const list = await listCleanIps(env)
    return json({ ok: true, ips: list })
  }
