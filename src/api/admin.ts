...
import {
  ...
  getUserByUuid,
  getBestCleanIp,
  addToProxyIpPoolFromScan,
} from '../db/queries'
import { scanSocks4, scanSni, scanCandidates, expandCidr, probeCleanIp, SNI_PORTS, MAX_CANDIDATE_JOBS } from '../services/ip-scanner'
...
/**
 * The domain used as the SNI/Host for every clean-IP probe (config-ping,
 * `/clean-ips/scan`, `/clean-ips/scan-candidates`). This used to silently be
 * `url.hostname` — whatever host the admin happened to be browsing the
 * dashboard on — which broke the whole technique on a `*.workers.dev`
 * deployment (and on any custom domain that wasn't actually proxied/orange-
 * clouded through Cloudflare yet): every scan against that host came back
 * "unhealthy" no matter how good the candidate IP actually was.
 *
 * That restriction doesn't actually exist. `resolveOverride` only forces
 * *which IP* the TCP connection dials; Cloudflare's edge still routes the
 * request purely by the TLS SNI / HTTP Host header, exactly like it would
 * for a normal visitor — it doesn't matter whose Cloudflare account the
 * matching zone lives in. So the probe only needs *some* domain that is
 * already proxied through Cloudflare, not a domain the admin owns or has
 * connected to this worker. Any big, always-on, Cloudflare-fronted site
 * works (e.g. farsroid.com), and using one means the scanner works with zero
 * setup, on a bare workers.dev deployment included.
 *
 * `scan_domain` (editable from "⚙️ تنظیمات") lets the admin override this
 * with a domain of their choice if they ever want to — e.g. their own
 * custom domain, once it's actually added to Cloudflare and proxied — but
 * it is no longer required for the scanner to work.
 */
const DEFAULT_SCAN_DOMAIN = 'farsroid.com'

async function resolveScanDomain(env: Env, _url: URL): Promise<string> {
  const configured = (await getSetting(env, 'scan_domain')).trim()
  return configured || DEFAULT_SCAN_DOMAIN
}
...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // Reuses the same fetch()+resolveOverride probe the clean-IP scanner uses
  // (see probeCleanIp in services/ip-scanner.ts) so the result reflects the
  // config's actual dialed address/port, similar to how a VLESS client would
  // open it. It presents resolveScanDomain()'s SNI/Host (see that function's
  // comment) rather than the config's own `sni` value, since that's the
  // domain this probe actually knows to be Cloudflare-fronted and reachable
  // — the config's own address/port is still what's being tested, only the
  // TLS/Host layer is swapped in.
  if (path === '/config-ping' && method === 'POST') {
    const { address, port } = await readJson(request)
    if (!address || !String(address).trim()) return json({ error: 'آدرس الزامی است' }, 400)
    const portNum = Number(port) || 443
    const scanDomain = await resolveScanDomain(env, url)
    const result = await probeCleanIp(String(address).trim(), portNum, scanDomain)
    return json({ ok: true, result, testedDomain: scanDomain })
  }
  ...
  if (path === '/clean-ips' && method === 'POST') {
    ...
    await addToProxyIpPoolFromScan(env, [{ ip: String(ip).trim(), port: portNum }])
    const ips = await listCleanIps(env)
    return json({ ok: true, ips })
  }

  // Bulk variant used by the Radar/manual-scan/Vahid-scan "add selected" button: the
  ...
  // first, on every call — see db/queries.ts). The same IPs are folded into
  // proxy_ip_pool (addToProxyIpPoolFromScan) so a scan result actually
  // starts being used for the Cloudflare-relay fallback too, not just the
  // client-facing server list — see that function's comment for why.
  if (path === '/clean-ips/bulk' && method === 'POST') {
    ...
    const added = await addCleanIps(env, normalized)
    await addToProxyIpPoolFromScan(env, normalized.filter((it) => it.ip.trim()))
    const ips = await listCleanIps(env)
  ...
  if (path === '/clean-ips/scan' && method === 'POST') {
    ...
    const sni = await resolveScanDomain(env, url)
