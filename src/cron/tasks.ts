...
export async function runScheduledMaintenance(env: Env, domain?: string): Promise<void> {
  ...
  // Requirement #5's clean-IP discovery used to run here (once per hour, as
  // part of this same job). It now runs on its own much shorter cycle —
  // see maybeRunCleanIpScan in core/handler.ts — because re-verifying the
  // candidate list once an hour was too slow to track fast-moving ISP
  // blocking. `domain` is kept as a parameter for backward compatibility
  // with any code still calling this with a domain, but is unused here now.
  void domain
}
