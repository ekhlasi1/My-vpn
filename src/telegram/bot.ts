...
import { sendMessage, getChatMember, answerCallbackQuery, broadcastToAll } from './api'
import { buildWeeklyUpdateMessage } from '../cron/tasks'
...
export async function handleTelegramUpdate(env: Env, request: Request, update: any): Promise<void> {
  ...
  // ---------- Admin: manual broadcast to every user who has ever messaged the bot ----------
  // Usage:
  //   /broadcast <متن پیام>   -> sends the given text (HTML formatting allowed) to everyone
  //   /broadcast update       -> resends the canned weekly "servers updated / bill paid" message on demand
  if (isAdmin && rawText.startsWith('/broadcast')) {
    const body = rawText.slice('/broadcast'.length).trim()
    if (!body) {
      await sendMessage(
        token,
        chatId,
        'فرمت درست:\n<code>/broadcast متن پیام شما</code>\n\nیا برای ارسال همون پیام هفتگی «سرورها بروزرسانی شد»:\n<code>/broadcast update</code>',
      )
      return
    }
    const messageText = body === 'update' ? buildWeeklyUpdateMessage(settings.brand_name || 'BNDMAX VPN') : body
    const botUsers = await listBotUsers(env, 100000)
    const chatIds = botUsers.map((u) => u.telegram_id).filter(Boolean)
    if (chatIds.length === 0) {
      await sendMessage(token, chatId, 'هیچ کاربری برای ارسال پیام پیدا نشد.')
      return
    }
    await sendMessage(token, chatId, `⏳ در حال ارسال پیام به ${chatIds.length} کاربر...`)
    const { sent, failed } = await broadcastToAll(token, chatIds, messageText)
    await sendMessage(token, chatId, `✅ ارسال شد.\nموفق: ${sent}\nناموفق: ${failed}`)
    return
  }
