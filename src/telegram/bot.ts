...
/** Requirement #1: trial usage and VIP usage must always be shown as two separate figures, never merged. */
function formatUsageBlock(user: UserRow): string {
...
export async function handleTelegramUpdate(env: Env, request: Request, update: any): Promise<void> {
  ...
  if (text === '/usage') {
    ...
    const blocks = [trial, pro].filter((u): u is UserRow => !!u).map(formatUsageBlock)
