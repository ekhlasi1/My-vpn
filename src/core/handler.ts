  ...
  try {
    ...
    if (uuidLike && env.DB) {
      ...
      if (user) {
        ...
        // turns the "🌐 آی‌پی‌های سالم" toggle ON from the dashboard,
        // getBestCleanIp() starts returning the best IP currently marked
        // healthy in the `clean_ips` table (tested from Iranian probe nodes
        // via check-host.net — see services/clean-ips.ts), and every config
        // generated from that point on uses it immediately; no redeploy or
        // cache to wait on. Toggling it back OFF, or running dry (nothing
        // discovered yet), makes this resolve to `null` and configs fall
        // straight back to the worker's own host.
        const cleanIp = await getBestCleanIp(env)
