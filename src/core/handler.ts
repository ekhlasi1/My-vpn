...
import { ensureSchemaSafe, getLastSchemaError } from '../db/ensure-schema.ts'
...
export async function handleRequest(
  ...
  try {
    ...
    // Self-heal the D1 schema (missing tables/columns/indexes after a GitHub
    // redeploy over an older database) — a boolean check after the first
    // successful run per isolate; never throws. See src/db/ensure-schema.ts.
    if (env.DB) await ensureSchemaSafe(env)
    ...
    if (url.pathname.startsWith('/api/admin/')) {
      try {
        ...
        // Previously an exception here fell through to the HTML error page,
        // so the dashboard could only ever show a generic "خطا". Answer with
        // JSON carrying the real reason (D1 errors such as "no such table")
        // so the admin sees — and can report — what actually failed.
        console.error('Admin API error:', request.method, url.pathname, err)
        const msg = String((err as Error)?.message ?? err).slice(0, 300)
        const schemaErr = getLastSchemaError()
        return new Response(
          JSON.stringify({
            error: 'خطای سرور/دیتابیس: ' + msg + (schemaErr ? ' — ترمیم خودکار دیتابیس ناموفق بود: ' + schemaErr.slice(0, 200) : ''),
          }),
