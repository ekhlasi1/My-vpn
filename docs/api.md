# BNDMAX VPN API Documentation

This document describes the API endpoints provided by BNDMAX VPN.
...
BNDMAX VPN inherits rate limiting from Cloudflare Workers:
...
BNDMAX VPN does not implement CORS headers as it's primarily designed for proxy traffic rather than web API usage.
...
As BNDMAX VPN evolves, endpoints may be added or modified. Breaking changes will be documented in the changelog and version releases.
...
This API documentation provides a complete reference for integrating with and using BNDMAX VPN.