# BNDMAX VPN Architecture

This document describes the architecture of BNDMAX VPN, a VLESS proxy server built for Cloudflare Workers.

## Overview

BNDMAX VPN is designed as a lightweight, high-performance proxy server that runs on Cloudflare's edge network. It leverages Cloudflare Workers' serverless architecture to provide global, low-latency access to proxy services.
